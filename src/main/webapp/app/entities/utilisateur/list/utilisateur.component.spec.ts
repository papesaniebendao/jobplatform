...
import { sampleWithRequiredData } from '../utilisateur.test-samples';
...
describe('Utilisateur Management Component', () => {
  ...
  it('should call load all on init', () => {
    ...
    expect(comp.utilisateurs()[0]).toEqual(expect.objectContaining({ id: 2179 }));
  });

  describe('trackId', () => {
    it('should forward to utilisateurService', () => {
      const entity = { id: 2179 };
      jest.spyOn(service, 'getUtilisateurIdentifier');
      const id = comp.trackId(entity);
      expect(service.getUtilisateurIdentifier).toHaveBeenCalledWith(entity);
