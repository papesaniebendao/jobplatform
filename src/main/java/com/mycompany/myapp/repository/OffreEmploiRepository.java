...
public interface OffreEmploiRepository extends ReactiveCrudRepository<OffreEmploi, Long>, OffreEmploiRepositoryInternal {
    ...
    @Query("""
        ...
        WHERE
            (:texte IS NULL OR (LOWER(o.titre) LIKE LOWER(CONCAT('%', :texte, '%')) OR LOWER(o.description) LIKE LOWER(CONCAT('%', :texte, '%'))))
            AND (:typeContratId IS NULL OR o.type_contrat_id = :typeContratId)
